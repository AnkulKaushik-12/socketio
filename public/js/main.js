...
class MainChat  {
	...
	static LoadEventHandlers() {
			Fake = [ 'Hi there, I\'m Ankul and you?', 'Nice talking you', 'How are you?', 'Not too bad, thanks', 'What do you do?', 'That\'s awesome', 'Antino is a nice place to stay', 'I think you\'re a nice person', 'Why do you think that?', 'Can you explain?', 'Anyway I\'ve gotta go now', 'It was a pleasure chat with you', 'Bye', ':)' ]
			...
			socket.on('updatechat',  (username, data) => {
				if(username == 'Chat Bot'){
					$('<div class="message loading new"><figure class="avatar"><img src="/public/img/india.png" /></figure><span></span></div>').appendTo($('.mCSB_container'));
					MainChat.updateScrollbar();
					setTimeout( () => {
					$('.message.loading').remove();
					$('<div class="message new"><figure class="avatar"><img src="/public/img/india.png" /></figure>' + data + '</div>').appendTo($('.mCSB_container')).addClass('new');
						MainChat.setDate();
						MainChat.updateScrollbar();
	...
	static fakeMessage() {
		...
		$('<div class="message loading new"><figure class="avatar"><img src="/public/img/india.png" /></figure><span></span></div>').appendTo($('.mCSB_container'));
			...
			setTimeout( () => {
					$('.message.loading').remove();
					$('<div class="message new"><figure class="avatar"><img src="/public/img/india.png" /></figure>' + Fake[i] + '</div>').appendTo($('.mCSB_container')).addClass('new');
						MainChat.setDate();
						MainChat.updateScrollbar();
						i++;
			}, 1000 + (Math.random() * 20) * 100);
